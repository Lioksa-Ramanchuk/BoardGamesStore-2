...
export async function handleAddItem(req, res, next) {
  ...
  const item = await itemService.create(title, description ?? '', rules ?? '', price, quantity, category ?? '', image, publisher, year, min_players, max_players, avg_play_time, player_min_age, is_available === 'on');
  if (!item) {
