...
export function generateAccessToken(accountId, accountRole, accountUuid) {
  return jsonwebtoken.sign({ account: { id: accountId, role: accountRole, uuid: accountUuid } }, process.env.JWT_SECRET, { expiresIn: +process.env.JWT_ACCESS_TOKEN_EXPIRES_IN });
}
...
export function generateRefreshToken(accountId, accountRole, accountUuid) {
  return jsonwebtoken.sign({ account: { id: accountId, role: accountRole, uuid: accountUuid } }, process.env.JWT_REFRESH_SECRET, { expiresIn: +process.env.JWT_REFRESH_TOKEN_EXPIRES_IN });
}
